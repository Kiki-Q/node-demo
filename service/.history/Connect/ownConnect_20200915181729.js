...
// morgan.token('query-string', (req, res)=> {
//         return url.parse(req.url).query;
// })

// let app = connect()
//         .use(morgan(':query-string'))
//         .listen(3000)


//stream写日志
let log = fs.createWriteStream('./log/myapp.log', {flags: 'a'});
...
// let app = connect()
//         .use(morgan({ immediate: true}))
//         .listen(3000)


connect().use(favicon(__dirname + '/img/favicon.png'))
        .use(morgan())
        .use((req, res)=>{
                res.end('thanks!' +__dirname);
        })
        .listen(3000)